...
1. Read `.loops/specs/{{ARGS}}.json`. If missing, list the specs in
   `.loops/specs/` and stop.
2. Read `.loops/CONTEXT.md` for repo grounding.
3. Create `.loops/state/{{ARGS}}.STATE.md` with sections: Goal, Rules, Inputs,
...
### Maker phase
Write `.loops/.phase-lock.json` as `{"slug": "{{ARGS}}", "phase": "maker",
